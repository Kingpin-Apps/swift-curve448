...
struct SecureBytes {
    ...
    init(count: Int) {
        if count == 0 {
            self.backing = SecureBytes.Backing.createEmpty()
        } else {
...
//    }
    ...
    @usableFromInline
    init(unsafeUninitializedCapacity: Int, initializingWith callback: (UnsafeMutablePointer<OpaquePointer?>?, inout Int) throws -> Void) rethrows {
        self.backing = Backing.create(capacity: unsafeUninitializedCapacity)
        
        try self.backing._withVeryUnsafeMutableBytes { veryUnsafePointer in
            guard let baseAddress = veryUnsafePointer.baseAddress else {
                throw CryptoKitError.incorrectKeySize
            }
            let opaquePointer = OpaquePointer(baseAddress)
            var initializedCount = 0
            try callback(UnsafeMutablePointer(opaquePointer), &initializedCount)
            
            // Ensure we don't exceed the allocated capacity
            precondition(initializedCount <= unsafeUninitializedCapacity, "Initialized count exceeds allocated capacity")
